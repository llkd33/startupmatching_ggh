...
-- Campaigns (FIXED: Specify table alias to avoid ambiguity)
CREATE POLICY "Organizations can manage own campaigns" ON public.campaigns
    FOR ALL USING (
        organization_id IN (
            SELECT op.id FROM public.organization_profiles op WHERE op.user_id = auth.uid()
        )
    );
...
-- Proposals (FIXED: Specify table alias to avoid ambiguity)
CREATE POLICY "Experts can manage own proposals" ON public.proposals
    FOR ALL USING (
        expert_id IN (
            SELECT ep.id FROM public.expert_profiles ep WHERE ep.user_id = auth.uid()
        )
    );
