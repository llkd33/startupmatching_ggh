import { toast, ERROR_MESSAGES as TOAST_ERROR_MESSAGES } from '@/components/ui/toast-custom'
...
export const errorHandler = {
  ...
  handle: (error: any, showToast: boolean = true): AppError => {
    ...
    if (showToast) {
      if (description) {
        toast.error(userMessage, typeof description === 'string' ? description : undefined)
      } else {
